...
$("body").delegate( ".borderOut", "click", function() {
	...
	$(".borderOut").removeClass("borderOutClicked");
	let id_Out = $(this).attr('id');
	const idSplit = id_Out.split("outil");
	id_Out = idSplit[1];
	const divInfosOutil = `#infos${id_Out}`;
	const idChevron = `#chevron${id_Out}`;
	// ------------------------------------------------------------------------- Récupération du display (flex ou none)
	const display = $(divInfosOutil).css("display");
	if (display === "flex") $(divInfosOutil).addClass("d-none");
	else {
		$(".borderOutInfos").addClass("d-none");
		$(divInfosOutil).removeClass("d-none");
		$(idChevron).addClass("rotate");
		$(this).addClass("borderOutClicked");
	}
