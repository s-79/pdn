...
// --------------------------------------------------------------------------------- AU DÉMARRAGE
$(function(){
	// ----------------------------------------------------------------------------- L'utilisateur a-t-il un identifiant de session et vient-il de la page connexion ?
	// const provenance = document.referrer;
	// // Localhost
	// if(id_pdn!="" && provenance ==="http://localhost/pdn/connexion.php") $("section").toggleClass("d-block");
	// // En ligne
	// // if(id_pdn!="" && provenance ==="http://anomalies.fr/connexion.php") $("section").toggleClass("d-block");
	// else {
	// alert("Merci de vous identifier pour pouvoir accéder à cette page.")
	// window.location.href="connexion.php";
	// }

	// -------------------------------------------------------------------------------------------------------------- Mettre en valeur le menu actif et en vert l'icone user si connecté
	...
	// $("#nomPdn").val("");
	// const xmlPdn = getXmlBase("php/sqlxml.php","v_promeneur_structure","id_pdn",id_pdn);
	// const nomPdn=getElement(xmlPdn,"v_promeneur_structure","id_pdn",id_pdn,"prenom_nom");
  // $("#nomPdn").val($("#nomPdn").val() + nomPdn);
...
$("#btnvalide").click(function(){
	...
	site_editeur=$("#site_editeur").val();
		...
		const ajoutOutil = outAjout("php/outAjoutPdn.php",nom,description,image,fichier,age,nom_editeur,site_editeur,valide,id_pdn);

		// ------------------------------------------------------------------------- Ajout des associations outil/thématique dans la table classer

		// ------------------------------------------------------------------------- Récupération de l'id de la ressource nouvellement créee
		const chaineSelectedOut=getXmlBase("php/sqlxml.php","outil","nom_outil",nom);
		...
		if (them2 !== "0") {const classerAjout = association("php/classerAjout.php",them2,idOutil);}
		if (them3 !== "0") {const classerAjout = association("php/classerAjout.php",them3,idOutil);}
		if (them4 !== "0") {const classerAjout = association("php/classerAjout.php",them4,idOutil);}
		if (them5 !== "0") {const classerAjout = association("php/classerAjout.php",them5,idOutil);}

    alert("La ressource a été correctement transmise et est en attente de validation.");
		location.reload();
	}
	else {
		location.reload();
...
// ----------------------------------------------------------------------------- Le champs thématique est-il bien rempli ?
const themOK = (them) => {
	if (them==="0") {
